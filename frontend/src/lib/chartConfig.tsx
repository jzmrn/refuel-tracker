...
/**
 * Normalizes the legend payload generated by Recharts:
 *
 * 1. Removes entries whose graphical item opted out via `legendType="none"`.
 *    Recharts overrides the per-item `legendType` with the `iconType` of the
 *    `<Legend>` element, which makes opted-out items reappear in the legend.
 * 2. Removes duplicate entries. Charts that render one graphical item per data
 *    segment (e.g. a price line split by gaps) would otherwise repeat the same
 *    series in the legend once per segment.
 */
export function normalizeLegendPayload(
  payload: readonly LegendPayloadEntry[] | undefined,
): LegendPayloadEntry[] {
  if (!payload) return [];
  ...
  return payload.filter((entry) => {
    const itemProps = entry.payload as { legendType?: string } | undefined;
    if (itemProps?.legendType === "none") return false;

    const key = `${String(entry.value)}|${entry.color ?? ""}`;
...
/**
 * Shared `<Legend content={...}>` renderer that hides opted-out and duplicate
 * series. Keeps the default Recharts rendering (icons, formatter, layout).
 */
export const renderChartLegend = (props: DefaultLegendContentProps) => {
  // Recharts passes its own `ref` through to the content renderer; it must not
  // be forwarded to DefaultLegendContent.
  const { ref: _ref, ...rest } = props;
  return (
    <DefaultLegendContent
      {...rest}
      payload={normalizeLegendPayload(props.payload)}
    />
  );
};

/** Props to spread on every `<Legend>` so duplicates are removed consistently. */
export const legendConfig: Pick<LegendProps, "content"> = {
